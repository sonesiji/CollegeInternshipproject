...
def reset_password(request, uidb64, token):
    try:
        ...
        logger.debug(f"User found: {user.username}")  # Debugging log
    except (TypeError, ValueError, OverflowError, User.DoesNotExist):
        logger.error("User does not exist or invalid UID.")  # Debugging log
        user = None

    if user is not None and CustomTokenGenerator().check_token(user, token):
        logger.debug("Valid token. Proceeding with password reset.")  # Debugging log
        if request.method == 'POST':
            new_password = request.POST.get('new_password')
            confirm_password = request.POST.get('confirm_password')
            ...
            if new_password == confirm_password:
                try:
                    logger.debug(f"Setting new password for user: {user.username}")  # Debugging log
                    user.set_password(new_password)
                    user.save()
                    ...
                    logger.error(f"Error saving password: {e}")  # Debugging log
                    messages.error(request, "There was an error saving your new password. Please try again.")
            else:
                messages.error(request, "Passwords do not match.")
        return render(request, 'reset_password.html')
    else:
        logger.error("Invalid reset link or user.")  # Debugging log
